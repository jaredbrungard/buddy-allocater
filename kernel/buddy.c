...
buddy_free(void *ptr)
{
    ...
    while (order < MAX_ORDER - 1) { // Stop if we reach 2048
...
static void
print_tree(struct header *block, int order, int level)
{
    ...
    if (block->size == size) {
        ...
        if (block->magic == MAGIC_USED) {
            ...
            // This can happen if the block is split, but we are
            // printing its *buddy* which is whole.
            // Or, it's the first half of a split.
            // Let's re-check logic.
            // Ah, the issue is `block` is always the *first* half.
            // We need to check the block at this *address*.
            struct header *current = (struct header *)block;
            if(current->size == size && current->magic == MAGIC_USED)
                printf("%s└──── used (%d)\n", indent, size);
            else if (current->size == size && current->magic == MAGIC_FREE)
                printf("%s└──── free (%d)\n", indent, size);
            else
                printf("%s└──── split... (ERROR)\n", indent); // Should not happen
        }
    } else {
        // This block is split
        ...
        // Print top half (buddy, higher address) first
        print_tree(buddy, next_order, level + 1);
        
        printf("%s   ───┤\n", indent);
        
        // Print bottom half (block, lower address)
        print_tree(block, next_order, level + 1);
    }
}
...
buddy_print(void *ptr)
{
    ...
    if(page->size == MAX_SIZE) {
        ...
        print_tree(buddy, MAX_ORDER - 1, 0);
        printf("───┤\n");
        print_tree(page, MAX_ORDER - 1, 0);
    }
}
...
buddy_test(void)
{
    ...
    // Final print to show the empty page (should be kfreed)
    // We need a valid pointer. Let's alloc and free.
    printf("\nfinal check (should be empty)\n");
    void *f = buddy_alloc(16);
    buddy_print(f);
    buddy_free(f);
